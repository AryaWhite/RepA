...
:wqfrom termcolor import cprint, colored
from transformers import AutoModelForCausalLM, AutoTokenizer
from pathlib import Path
import os

API_HOST = "https://api.huiyan-ai.cn/v1/"
API_KEY = "sk-SuqIbry5iDPm2nOfIlZerr27ZmhEyTu1cvNo1ofwU1Q0EIt3"
PUBLIC_MODEL = "/home/breaker/public_models"


MODEL_PATH = {
      "llama2": f"{PUBLIC_MODEL}/Llama-2-7b-chat-hf",
      "llama3.1": f"{PUBLIC_MODEL}/Meta-Llama-3.1-8B-Instruct",
      "llama3": f"{PUBLIC_MODEL}/Meta-Llama-3-8B-Instruct",
      "gemma2": f"{PUBLIC_MODEL}/gemma-2-9b-it",
      "vicuna": f"{PUBLIC_MODEL}/vicuna-7b-v1.5",
      "qwen3": f"{PUBLIC_MODEL}/Qwen3-8B",
      "qwen2.5": f"{PUBLIC_MODEL}/Qwen2.5-7B-Instruct",
      "baichuan": f"{PUBLIC_MODEL}/Baichuan-7B",
      "hunyuan": f"{PUBLIC_MODEL}/Hunyuan-7B",
...
class LLM(ABC):
    ...
    def conv2prompt(self, conversation: List[Dict], verbose=False, thinking=True) -> str:
        ...
        if self.tokenizer.chat_template is None:
            ...
            if not thinking and re.match("^(qwen3|hunyuan)", self.model_name.lower()):
