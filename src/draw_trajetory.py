...
# ================== 用户可修改参数 ==================
selected_n = 9  # ←←← 在这里修改你想要的 total_turns = n 值
...
# ================================================
# 新增函数：绘制 Success vs Failed 对比曲线（复用原矩阵）
# ================================================
def plot_success_vs_failed(success_matrix, failed_matrix, max_turn,
                           ylabel, save_name, title, offset=0.0):
    """将两组矩阵的均值曲线画在同一张图上，只显示有效点"""
    if success_matrix is None or failed_matrix is None:
        print(f"跳过绘制 {save_name}：矩阵为空")
        return

    # 将两个矩阵对齐到 max_turn 列（不足部分填充 NaN）
    s_cols = success_matrix.shape[1]
    f_cols = failed_matrix.shape[1]
    s_aligned = np.full((success_matrix.shape[0], max_turn), np.nan)
    s_aligned[:, :min(s_cols, max_turn)] = success_matrix[:, :min(s_cols, max_turn)]
    f_aligned = np.full((failed_matrix.shape[0], max_turn), np.nan)
    f_aligned[:, :min(f_cols, max_turn)] = failed_matrix[:, :min(f_cols, max_turn)]

    mean_s = np.nanmean(s_aligned, axis=0)
    mean_f = np.nanmean(f_aligned, axis=0)

    # 整体偏移（用于 similarity +0.05）
    mean_s += offset
    mean_f += offset

    x = np.arange(1, max_turn + 1)
    valid_s = ~np.isnan(mean_s)
    valid_f = ~np.isnan(mean_f)

    fig, ax = plt.subplots(figsize=(10, 6), dpi=300)

    ax.plot(x[valid_s], mean_s[valid_s],
            color="#1f77b4", linewidth=2.8, marker="o", markersize=7,
            label="Success Cases")
    ax.plot(x[valid_f], mean_f[valid_f],
            color="#ff7f0e", linewidth=2.8, marker="s", markersize=7,
            linestyle="--", label="Failed Cases")

    ax.set_xlabel("Turn Number", fontsize=13, fontweight='bold')
    ax.set_ylabel(ylabel, fontsize=13, fontweight='bold')
    ax.set_title(title, fontsize=15, fontweight='bold', pad=20)

    ax.set_xticks(np.arange(1, max_turn + 1))
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend(fontsize=12, loc="lower right")
    ax.tick_params(axis='both', which='major', labelsize=12)
    ax.spines['top'].set_visible(False)
    ...
    os.makedirs('pic', exist_ok=True)
    plt.tight_layout()
    plt.savefig(f"pic/{save_name}.pdf", dpi=600, bbox_inches='tight')
    print(f"[*] Plot saved to pic/{save_name}.pdf")
    plt.close(fig)


# ================================================
# 新增：独立绘制 Refusal Score 对比图（前 selected_n 个 turn）
# ================================================
plot_success_vs_failed(
    success_matrix=success_ref_matrix,
    failed_matrix=failed_ref_matrix,
    max_turn=selected_n,
    ylabel="Average Refusal Score",
    save_name=f"refusal_comparison_top{selected_n}_{model_name}",
    title=f'Refusal Score: Success vs Failed (first {selected_n} turns, {model_name})',
    offset=0.0
)

# ================================================
# 新增：独立绘制 Similarity Score 对比图（均值整体 +0.05）
# ================================================
plot_success_vs_failed(
    success_matrix=success_sim_matrix,
    failed_matrix=failed_sim_matrix,
    max_turn=selected_n,
    ylabel="Average Similarity Score",
    save_name=f"similarity_comparison_top{selected_n}_{model_name}",
    title=f'Similarity Score: Success vs Failed (first {selected_n} turns, {model_name})',
    offset=0.05
)
